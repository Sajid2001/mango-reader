  ...
  const SearchManga = (term: string) => {
    let link =
      term == ""
        ...
        : `http://127.0.0.1:8000/api/manga/search?name=${term}`;
    fetch(link)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        ...
        <div className="flex m-1 *:mr-2">
          <div className="flex relative max-w-30">
            <input
              ...
              className="py-1 mb-3 grow font-semibold text-lg px-5 pl-10 bg-secondary rounded-lg placeholder:text-text"
              placeholder="Search Library..."
            />
            <button
              onClick={() => SearchManga(searchTerm)}
            ...
            <div className="flex justify-center">
              ...
              >
                {!isRefreshing ? (
                  ...
                  <svg
                    aria-hidden="true"
                    className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-400 fill-slate-800"
                    viewBox="0 0 100 101"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
